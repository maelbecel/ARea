...
public class TwitchNewFollowerAction extends Action {

    public TwitchNewFollowerAction(String appName) {
        super(appName, "New follower", "New follower on your channel");

        this.inputs.add(FormInput.createTextInput("channel", "Your channel name"));

        this.placeholders.put("username", "The username of the new follower");
    }
    ...
    public void setupAction(User user, List<FormInput> inputs) throws ActionTriggerException {
        ...
        try {
            ...
            if (response.getStatus() == 401 || response.getStatus() == 403) {
                throw new ActionTriggerException("Fail to connect to Twitch - " + response.getData());
            }
